...
Some numerical variables seemed to be skewed such as: "INCOME", "TRAVTIME", "OLDCLAIM", "HOME_VAL" and "BLUEBOOK" were skewed to the right. We did a log tranformation to make the variables have a more normally distributed data which is importan to have as algorithms assume that data is normally distributed. To ensure we tranforms the skewed numeric variables that are not count variables we used log transformation. For the transformation we just added new variables just in case we wanted to use the non-transformed variable for modeling.
...
Logistic model for Target flag using all variables, as target flag is a binary variable. This model could be used for predicting crash probability.
```{r}
logist_model<-glm(TARGET_FLAG ~., data=train_df_imp, family=binomial)
summary(logist_model)
```

Reduced logistic regression model.
```{r}
logist_model<-glm(TARGET_FLAG ~ AGE+BLUEBOOK_LOG+INCOME_LOG+HOME_VAL_LOG+OLDCLAIM_LOG+TRAVTIME_LOG+CLM_FREQ+CAR_AGE, data=train_df_imp, family=binomial)
summary(logist_model)
```

Logistic model without transformed data.
```{r}
logist_model<-glm(TARGET_FLAG ~ AGE+BLUEBOOK+INCOME+HOME_VAL+OLDCLAIM+TRAVTIME+CLM_FREQ+CAR_AGE, data=train_df_imp, family=binomial)
summary(logist_model)
```
