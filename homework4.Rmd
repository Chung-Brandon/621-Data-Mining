...
Since Logistic Target flag variable is binary 3 logistic models were used to predict target flag.
 
The first model logistic model uses all variables except Target amt, as it is defined for claimana and should be used to predict the likelihood someone would crash. This model could be used for predicting crash probability.
The coefficient show change in crash probability with each predictor, a positive coefficients means a higher crash probability and a negative coefficent would mean a lower crash probability. OLDClaim_LOG, MVR_PTS and CLM_FREQ had positive variables as expected.


```{r}
logist_model1<-glm(TARGET_FLAG ~.-TARGET_AMT, data=train_df_imp, family=binomial)
...
```{r}
test_probs <- predict(logist_model1, newdata = test_df_imp, type = "response")
...
```{r}
test_probs2 <- predict(logist_model2, newdata = test_df_imp, type = "response")
test_class2 <- ifelse(test_probs2 > 0.5, 1, 0)
...
Logistic model without transformed data. This model uses the same core predictors as Model 2 but without log transformations, to compare whether transformations materially improve interpretability or predictive accuracy. Log transformations were applied to reduce skewness (like OLDCLAIM, HOME VAL, BLUEBOOK), improving coefficient stability. CAR USE and CAR TYPE were included because exploratory analysis indicated differences in crash likelihood across usage categories and vehicle types. Based on these coeeficient OLDCLAIM and CLM_FREQ positively associated with crash probability and AGE, HOMEVAL, and BLUEBOOK negatively associated, suggesting older, wealthier drivers are safer. Also, the car type show the bigger the vechile the higher the probability the person will crash.
```{r}
logist_model3<-glm(TARGET_FLAG ~ AGE+BLUEBOOK+INCOME+HOME_VAL+OLDCLAIM+TRAVTIME+CLM_FREQ+CAR_AGE+CAR_TYPE, data=train_df_imp, family=binomial)
summary(logist_model3)
```

```{r}
test_probs3 <- predict(logist_model3, newdata = test_df_imp, type = "response")
test_class3 <- ifelse(test_probs3 > 0.5, 1, 0)
