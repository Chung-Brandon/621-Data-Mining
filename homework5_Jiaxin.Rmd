...
The dataset contains 16 columns and 12,795 rows. An initial review shows a number of missing values, with 8 columns containing NAs. We also observe that several numerical features include negative values.
...
#### Missing Values
We use mice package to impute the missing values in the dataset.
...
Many numerical features have somewhat unreasonable negative values, so we decided to use log-transformed variables. 
